...
### Method: Single Web Service (Recommended)
This method serves both the Frontend (React) and Backend (FastAPI) from one service.
...
5.  **Environment Variables:** Add keys from your local `.env`.
    *   `QDRANT_URL`, `QDRANT_API_KEY`, `GROQ_API_KEY`, `SECRET_KEY`
...
### Step 1: Configure Qdrant
Ensure your `QDRANT_URL` and `QDRANT_API_KEY` are correctly set in the `.env` file. We recommend using **Qdrant Cloud** for easiest setup.
