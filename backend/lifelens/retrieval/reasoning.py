...
def get_answer(query: str, memories: list) -> str:
    ...
    for idx, mem in enumerate(memories):
        collection_name = mem.get('collection_name', 'unknown')
        ...
        elif mem_type in ('person', 'object', 'patient', 'enrollment_profile'):
            enrollment_bits = [
                f"Name: {mem.get('name')}",
                f"Relation: {mem.get('relation')}" if mem.get('relation') else None,
                f"Notes: {mem.get('notes')}" if mem.get('notes') else None,
                f"Age: {mem.get('age')}" if mem.get('age') is not None else None,
            ]
            content = " | ".join(bit for bit in enrollment_bits if bit)
        ...
        memory_context += f"{idx+1}. [{collection_name} :: {str(mem_type).upper()}] {content}{person_info}{location_info} (Timestamp: {timestamp})\n"
...
11. If evidence comes from multiple Qdrant collections, organize the answer by collection or topic.
12. If the query is about an enrolled person or object, prefer the enrollment collection evidence when it is relevant.
...
Format the response in a concise structured way using short headings or bullet-like sections when helpful.
"""
