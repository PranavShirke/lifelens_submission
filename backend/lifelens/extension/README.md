...
The extension works **INDEPENDENTLY of the Next.js dashboard** - you can close the dashboard and the extension will continue to function as long as the API server is running.
...
Browser Extension → FastAPI Server (port 8000) → Qdrant Database
                    ↑
Next.js Dashboard (port 3000) - Optional, can be closed
```
...
**Option B: API Server Only (for extension)**
```bash
cd backend
.\.venv\Scripts\python.exe -m uvicorn lifelens.api.main:app --host 0.0.0.0 --port 8000 --reload
```

**Option C: Manual dashboard start**
```bash
cd frontend
npm run dev
```
...
- **Solution**: Run `start_lifelens.bat`
...
## API Server vs Dashboard

| Component | Port | Purpose | Required for Extension? |
|-----------|------|---------|------------------------|
| API Server | 8000 | Handle extension requests | ✅ **YES** |
| Next.js Dashboard | 3000 | Web UI for patients/caretakers | ❌ No |
| Qdrant | 6333 | Vector database | ✅ **YES** |

**Key Point**: You can close the dashboard and the extension will continue to work as long as:
...
1. ✅ Start API server: `start_lifelens.bat`
2. ✅ Load extension in browser
3. ✅ Login with patient account (patient1 / patient123)
4. ✅ Start saving memories!
5. ❌ Close the dashboard if desired (extension still works!)
