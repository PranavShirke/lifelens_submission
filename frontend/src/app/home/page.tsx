...
function PatientHomeContent() {
  ...
  const handleToggleMedication = async (medId: string, currentTime: string, currentStatus: string) => {
    ...
    try {
      ...
      const knownError = error as { response?: { data?: { detail?: string } }; message?: string };
      console.error('[Medication] Update failed:', knownError.response?.data || knownError.message);
      setMedEvents(previousEvents);
      addToast({ 
        type: 'error', 
        message: knownError.response?.data?.detail || 'Failed to update medication status' 
      });
    }
  };
  ...
  const StatContent = ({ s, adherence }: { s: StatCard; adherence: number }) => (
    <>
                    ...
                    <div key={`${med.medicationId}-${med.doseTime}`} className="p-3 rounded-xl hover:bg-[#FF8C42]/10 transition-colors group">
                      <div className="grid grid-cols-[15%_25%_45%_15%] gap-2 items-center">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-[#9896B0]/60 font-medium truncate">{med.note || 'Scheduled dose'}</p>
                        </div>
