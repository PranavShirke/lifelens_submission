...
from lifelens.config import QDRANT_COLLECTION_NAME, QDRANT_USER_COLLECTION_NAME, GEMINI_API_KEY
...
def search_memories(client: QdrantClient, query: str, filters: dict = None, top_k: int = 10, patient_id: str = None):
    ...
    enrollment_keywords = ['enroll', 'enrollment', 'registered', 'remember/person', 'remember/object']
    ...
    is_enrollment_query = any(keyword in query_lower for keyword in enrollment_keywords)
    ...
    collections_to_search = SEARCH_COLLECTIONS[:]
    if QDRANT_USER_COLLECTION_NAME in collections_to_search:
        collections_to_search.remove(QDRANT_USER_COLLECTION_NAME)
    ...
    for collection_name in collections_to_search:
        try:
            ...
            logging.debug(f"Skipping collection {collection_name}: {exc}")
            continue
        ...
        for hit in search_result:
            ...
            result = {
                "collection_name": collection_name,
                "score": hit.score,
                "type": payload.get("type") or payload.get("enrollment_type"),
                "caption": payload.get("caption"),
                "transcript": payload.get("transcript"),
                "content": payload.get("content") or payload.get("name"),
                "analysis": payload.get("analysis"),  # For video
                "timestamp": payload.get("timestamp"),
                "sentiment": payload.get("sentiment"),
                "person_tags": payload.get("person_tags") or payload.get("relation"),
                "location": payload.get("location"),
                "source_image_base64": payload.get("source_image_base64") or payload.get("image_base64"),
                "source_audio_base64": payload.get("source_audio_base64") or payload.get("audio_base64"),
            ...
            # Bias toward enrollment collection when user explicitly asks enrollment-related questions.
            if is_enrollment_query and collection_name == "lifelens_enrollment":
                keyword_boost += 0.15
