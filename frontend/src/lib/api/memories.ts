...
function mapBackendMemory(m: Record<string, unknown>): Memory {
  ...
  const backendType = (m.type as string | undefined) || 'text';
  const hasImage = Boolean(m.image_base64 || m.source_image_base64);
  const normalizedType: Memory['type'] =
    backendType === 'image' || backendType === 'audio' || backendType === 'text'
      ? backendType
      : hasImage
        ? 'image'
        : 'text';
