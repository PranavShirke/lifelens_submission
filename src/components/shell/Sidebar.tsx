...
import {
  ...
  MessageSquare, Pill, MapPin, Settings,
  LogOut, LayoutDashboard, FileText, HeartPulse, ChevronRight
} from 'lucide-react';
...
const generalItems: NavItem[] = [
  ...
  { label: 'Patients', href: '#', icon: User, roles: ['caretaker'] },
...
const toolItems: NavItem[] = [
  ...
  { label: 'Medications', href: '/medications', icon: Pill, roles: ['patient', 'caretaker'] },
...
export default function Sidebar() {
      ...
      <Link href={item.href} className="block relative group">
        {active && (
          <motion.div 
            ...
            className="absolute left-0 top-[6px] bottom-[6px] w-[3px] rounded-r-full bg-[#9B8EC4]"
        ...
        <div className={cn(
          ...
          active
            ? 'text-white bg-[#9B8EC4]/15'
            : 'text-[#9896B0] hover:text-white hover:bg-[#9B8EC4]/5'
        )}>
          <item.icon className={cn('w-4 h-4 stroke-[1.7] flex-shrink-0', active && 'text-[#9B8EC4]')} />
          <span className="flex-1">{item.label}</span>
          {active && <ChevronRight className="w-3.5 h-3.5 text-[#9B8EC4] opacity-60" />}
        </div>
      </Link>
    );
  };
  ...
  return (
    <aside className="w-[240px] h-full flex flex-col border-r border-[#9B8EC4]/10 relative z-50" style={{ background: '#1E1B2E' }}>
      
      {/* Logo */}
      <div className="px-5 pt-6 pb-4 border-b border-[#9B8EC4]/10">
      ...
      {/* User */}
      <div className="px-4 py-3 border-b border-[#9B8EC4]/10">
        <div className="flex items-center gap-2.5 px-1">
          <div 
            className="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0"
            style={{ background: 'linear-gradient(135deg, #9B8EC4, #7A9E7A)' }}
          >
            {firstName.charAt(0)}
          </div>
      ...
      <nav className="flex-1 overflow-y-auto pt-3 custom-scrollbar">
        ...
        {/* Divider */}
        <div className="mx-5 my-2 border-t border-[#9B8EC4]/10" />
      ...
      {/* Logout */}
      <div className="px-4 py-3 border-t border-[#9B8EC4]/10">
        <button
          onClick={() => {
            logout();
