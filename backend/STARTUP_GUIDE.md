...
Before first run:
- Ensure `backend\.env` contains `GROQ_API_KEY`, `GEMINI_API_KEY`, `QDRANT_URL`, and `QDRANT_API_KEY`
- The one-click launcher seeds `backend\.env` and `backend\lifelens\.env` automatically
...
The launcher bootstraps the backend Python environment and installs frontend dependencies automatically on first run.
...
**What it does:**
- Opens main LifeLens dashboard at `http://localhost:3000`
- Patient/Caretaker/Family portal
- Memory upload, search, medications, mood tracking

**Wait for:** Next.js compilation to finish and the browser to open
...
```bash
cd backend
.\.venv\Scripts\python.exe lifelens\scripts\medication_scheduler_service.py
```
