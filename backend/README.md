...
🔗 **Live Demo:** [https://lifelens-lls.streamlit.app/](https://lifelens-lls.streamlit.app/) (the final version is not deployed, this version is the first prototype and it might be broken)
...
3. Start the full demo stack (recommended):
...
- create a local virtual environment (`.venv`) if needed
- install dependencies from `requirements.txt` on first run
- seed `lifelens/.env` from root `.env` if missing
- launch API (`8000`), Streamlit (`8501`), and Medication Scheduler

4. If you prefer manual service startup:
...
QDRANT_URL=
QDRANT_API_KEY=
# Optional avatar local Qdrant overrides (only if QDRANT_URL is not set)
# QDRANT_MODE=local
# QDRANT_HOST=localhost
# QDRANT_PORT=6333
# QDRANT_PATH=qdrant_storage
# Optional avatar collection names (safe defaults avoid collisions)
# AVATAR_FACES_COLLECTION=lifelens_avatar_faces
# AVATAR_OBJECTS_COLLECTION=lifelens_avatar_objects
# AVATAR_PATIENTS_COLLECTION=lifelens_avatar_patients
# AVATAR_KNOWLEDGE_COLLECTION=lifelens_avatar_text_knowledge
GEMINI_API_KEY=
GROQ_API_KEY=
JWT_SECRET=
NTFY_TOPIC_URL=
