...
import {
  mockMedications, mockMemories, mockAgentSuggestions, mockMoodDistribution,
  mockFamilyRequests, mockWeeklyActivity
} from '@/lib/mock-data';
import {
...
function DashboardContent() {
  ...
  const filteredRequests = mockFamilyRequests.filter((r) => {
          ...
          <div className="card bg-gradient-to-br from-[#18181b] to-[#27272a] text-white border-white/10 shadow-2xl relative overflow-hidden h-[420px] group hover:shadow-[0_0_40px_-10px_rgba(244,114,182,0.4)] transition-shadow duration-500">
             ...
             <div className="relative z-10 flex flex-col h-full">
               ...
               <div className="space-y-3 flex-1 overflow-y-auto pr-1 custom-scrollbar">
                 {mockAgentSuggestions.slice(0, 3).map((s, i) => (
                   <motion.div 
                     key={s.id} 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 h-[420px] md:h-[260px]">
             ...
             <div className="card h-full shadow-lg p-5 group hover:shadow-xl transition-all duration-300">
               ...
               <div className="flex-1 min-h-0 w-full group-hover:scale-[1.02] transition-transform duration-500 origin-bottom">
                 <ResponsiveContainer width="100%" height="100%">
                   <BarChart data={mockWeeklyActivity.slice(-14)} margin={{ left: -20, right: 0, top: 0, bottom: 0 }}>
             ...
             <div className="card h-full shadow-lg p-5 group hover:shadow-xl transition-all duration-300">
               ...
               <div className="flex-1 min-h-0 w-full -mt-2 group-hover:scale-[1.05] transition-transform duration-500">
                 <ResponsiveContainer width="100%" height="100%">
                   <PieChart>
                     <Pie data={mockMoodDistribution} dataKey="count" nameKey="emotion" cx="50%" cy="50%" innerRadius={40} outerRadius={70} paddingAngle={3}>
                       {mockMoodDistribution.map((entry) => (<Cell key={entry.emotion} fill={entry.color} />))}
                     </Pie>
        ...
        <motion.div variants={itemVariants} className="col-span-1 lg:col-span-3 flex flex-col gap-6">
          ...
          <div className="card shadow-lg h-[420px] lg:h-auto lg:flex-1">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-[10px] font-bold text-slate-500 bg-slate-100 shadow-inner px-2.5 py-1 rounded-lg">{mockMedications.length} items</span>
            </div>
            <div className="space-y-3 flex-1 overflow-y-auto pr-1 custom-scrollbar">
               {mockMedications.slice(0, 5).map((med, i) => (
                 <motion.div 
                   initial={{ opacity: 0, x: 20 }}
                   animate={{ opacity: 1, x: 0 }}
                   transition={{ delay: 0.3 + i * 0.1 }}
               ...
               <motion.button 
                 whileHover={{ scale: 1.02 }}
                 whileTap={{ scale: 0.98 }}
                 className="w-full py-3 text-[11px] font-bold uppercase tracking-wider text-slate-500 hover:text-slate-900 hover:bg-slate-100 rounded-xl transition-all border border-dashed border-slate-300 hover:border-solid mt-2"
               >
